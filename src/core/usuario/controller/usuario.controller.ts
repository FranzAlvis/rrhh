...
export class UsuarioController extends BaseController {
  ...
  async crear(@Req() req, @Body() usuarioDto: CrearUsuarioDto) {
    if (!usuarioDto.roles.includes('3')) {
      usuarioDto.roles.push('3')
    }
  ...
  @UseGuards(JwtAuthGuard, CasbinGuard)
  @Get('listar-para-permisos')
  async listarParaPermisos() {
    const result = await this.usuarioService.listarUsuariosParaPermisos()
    return this.successList(result)
  }
  ...
  async listarConsultores() {
    const result = await this.usuarioService.listarEscalaSalarialConsultor()
    return this.successList(result)
  }
}
