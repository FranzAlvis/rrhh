...
export class UsuarioService extends BaseService {
  ...
  protected tieneAlgunasProperiedadesConItem(usuarioDto: CrearUsuarioDto) {
    const {
    ...
    return (
      nroFuncionarioCarrera || nroRegistroProfesional || fechaAsignacionItem
    )
  }

  protected esPeronsalConItem(usuarioDto: CrearUsuarioDto) {
    const {
      nroFuncionarioCarrera,
      nroRegistroProfesional,
      fechaAsignacionItem,
  ...
  protected esPersonalMasculinoConItem(usuarioDto: CrearUsuarioDto) {
    return (
      this.esPeronsalConItem(usuarioDto) &&
      usuarioDto.persona.genero === Genero.MASCULINO
    )
  }

  protected esPersonalConLibretMilitar(usuarioDto: CrearUsuarioDto) {
    return usuarioDto.persona.nroLibretaServicioMilitar !== undefined
  }

  async crear(usuarioDto: CrearUsuarioDto, usuarioAuditoria: string) {
    if (
      this.tieneAlgunasProperiedadesConItem(usuarioDto) &&
      !this.esPeronsalConItem(usuarioDto)
    ) {
      throw new BadRequestException(Messages.EXISTING_ITEM_COMPLEED_FIELD)
    }
    if (
      this.esPersonalMasculinoConItem(usuarioDto) &&
      !this.esPersonalConLibretMilitar(usuarioDto)
    ) {
      throw new BadRequestException(Messages.EXISTING_ITEM_LIBRETA_MILITAR)
    }
    const contrasena = TextService.generateShortRandomText()
    const datosCorreo = {
      correo: usuarioDto.persona.correoElectronicoPersonal,
      ...
      const usuarioNuevo = await this.usuarioRepositorio.crear(
        {
          ...
          persona: {
            ...
            primerApellido: '',
            segundoApellido: '',
            nroDocumento: TextService.generateNro(),
            fechaNacimiento: new Date(),
            tipoDocumento: TipoDocumento.OTRO,
            correoElectronicoPersonal: usuarioDto.correoElectronico,
          },
  ...
  async crearConCiudadaniaV2(
    ...
    const nuevoUsuario: CrearUsuarioDto = {
      ...
      roles: rol?.id ? [rol?.id] : [],
      profesion: '',
      nroRegistroProfesional: '0',
      nroFuncionarioCarrera: '0',
    }
  ...
  async actualizarDatos(
    ...
    await this.usuarioRepositorio.actualizarUsuarioPersona(
      id,
      usuarioAuditoria,
      usuarioDto
    )
  ...
  async actualizarDatosPersona(datosPersona: PersonaDto | any) {
    return await this.usuarioRepositorio.actualizarDatosPersona(datosPersona)
  }

  async listarUsuariosParaPermisos() {
    return await this.usuarioRepositorio.listarUsuariosParaPermisos()
  }

  async listarEscalaSalarialConsultor() {
    return await this.usuarioRepositorio.listarEscalaSalarialConsultor()
  }
}
